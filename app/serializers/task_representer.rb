class TaskRepresenter
  class << self
    ...
    # Detailed task representation with pauses, time summary, and statistics
    def render(task)
    ...
    # Lightweight representation tailored for TasksController#show
    # Mirrors the previous controller as_json shape
    def render_show(task)
      return nil unless task
      {
        ...
        account: account_details_for(task),
        user: user_details_for(task)
      }
    end

    # Collection representation grouped by status (for index)
    ...
    # Non-memoized helpers for render_show to avoid reliance on @task
    def account_details_for(t)
      acc = t.account
      return nil unless acc
      { id: acc.id, name: acc.name }
    end

    def user_details_for(t)
      u = t.user
      return nil unless u
    ...
    # Helpers for collection rendering
    def initialize_buckets
      Task.statuses.keys.each_with_object({}) { |s, h| h[s] = [] }
    end

    def render_item(t)
      t.as_json(only: [:id, :title, :description, :priority, :status, :due_date_time, :pause_count, :total_working_time])
    end
  end
end

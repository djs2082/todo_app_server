class UserMailer < ApplicationMailer
  ...
  def send_template_email(user_email, template_name, variables = {}, subject: nil, from: nil)
    template = EmailTemplate.find_by(name: template_name)
    return unless template

    @variables = variables.with_indifferent_access

    subject_text = subject.presence || template.subject
    view_path = "email_templates/#{template_name}"
    if lookup_context.exists?(template_name, ['email_templates'], false, [:html])
      mail(to: user_email, subject: subject_text, from: (from if from.present?)) do |format|
